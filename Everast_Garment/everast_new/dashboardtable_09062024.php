...
$sql = "SELECT `u`.`product_code`, `u`.`product_name`, `ua`.`qty`, `ua`.`batchno`, `ua`.`update` FROM `tbl_stock` AS `ua` LEFT JOIN `tbl_product` AS `u` ON (`ua`.`tbl_product_idtbl_product` = `u`.`idtbl_product`) WHERE `u`.`status` ='1' ORDER BY 
`u`.`product_code` DESC";
