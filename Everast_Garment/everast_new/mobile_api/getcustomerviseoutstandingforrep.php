...
$sql = "SELECT `uc`.`name`, COALESCE(SUM(`u`.`nettotal`), 0) AS 'totalamount', COALESCE(SUM(`ue`.`payamount`), 0) AS 'totpayedamount'
        ...
        GROUP BY `u`.`tbl_customer_idtbl_customer`, `u`.`idtbl_invoice`";
$result = mysqli_query($con, $sql);
$dataarray = array();

while ($row = mysqli_fetch_array($result)) {
    array_push($dataarray, array("customername" => $row['name'], "fulltot" => $row['totalamount'], "payedamount" => $row['totpayedamount']));
}
