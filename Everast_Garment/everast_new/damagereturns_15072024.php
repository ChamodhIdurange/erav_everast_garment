...
$sqlreturndamageaccepted="SELECT `e`.`name` as `asm`, `u`.`idtbl_return`, `u`.`returndate`, `u`.`total`, `u`.`damaged_reason`, `ua`.`name` FROM `tbl_return` as `u` LEFT JOIN `tbl_customer` AS `ua` ON (`ua`.`idtbl_customer` = `u`.`tbl_customer_idtbl_customer`) JOIN `tbl_employee` AS `e` ON (`e`.`idtbl_employee` = `u`.`tbl_employee_idtbl_employee`) WHERE `u`.`acceptance_status` = '1' and `u`.`returntype` = '3' and `u`.`recieved_status` = '0'";
