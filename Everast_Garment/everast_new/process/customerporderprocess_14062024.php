    ...
    if ($conn->query($insretorder) == true) {
       ...
       // $deleterecode = " DELETE FROM `tbl_porder_detail` WHERE `tbl_porder_idtbl_porder` = '$recordID'";
        ...
        foreach ($tableData as $rowtabledata) {
            ...
            $insertorderdetail = "UPDATE `tbl_porder_detail` SET `type`='0', `qty`='$newqty', `freeqty`='$freeqty', `freeproductid`='$product', `unitprice`='$unitprice', `saleprice`='$saleprice', `status`='1', `updatedatetime`='$updatedatetime', `tbl_user_idtbl_user`='$userID', `tbl_product_idtbl_product`='$product' WHERE `idtbl_porder_detail`='$id'";
            $conn->query($insertorderdetail);
