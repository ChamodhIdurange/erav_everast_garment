...
if (!isset($_POST['monthend_ym']) || empty($_POST['monthend_ym'])) {
  $_SESSION['msg'] = "Month is required.";
  header("Location: ../monthend.php");
  exit;
}

$ym = $_POST['monthend_ym'];      
$monthFirst = $ym . "-01";
$monthDate  = date("Y-m-t", strtotime($monthFirst)); 
...
try {
  ...
  $stmt2->bind_param("si", $monthDate, $userId);
  $stmt2->execute();
  $stmt2->close();

  $conn->commit();
  $_SESSION['msg'] = "Month End successfully submitted for $pretty.";
} catch (Throwable $e) {
  $conn->rollback();
