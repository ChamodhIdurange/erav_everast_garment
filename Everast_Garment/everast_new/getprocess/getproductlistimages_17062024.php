...
$sqldetails = "SELECT `tbl_catalog_details`.`idtbl_catalog_details`, `tbl_product`.`product_name`, `tbl_product`.`idtbl_product`, `tbl_product`.`unitprice`,  `tbl_product`.`product_code`, SUM(`tbl_stock`.`qty`) AS `qty`, `tbl_product`.`tbl_sizes_idtbl_sizes`, `tbl_product`.`tbl_size_categories_idtbl_size_categories`, `tbl_sizes`.`name` AS `sizename`, `tbl_size_categories`.`name` AS `catname` FROM `tbl_catalog_details` LEFT JOIN `tbl_product` ON `tbl_product`.`idtbl_product` = `tbl_catalog_details`.`product_name` LEFT JOIN `tbl_stock` ON `tbl_stock`.`tbl_product_idtbl_product` = `tbl_product`.`idtbl_product` LEFT JOIN `tbl_sizes` ON `tbl_sizes`.`idtbl_sizes` = `tbl_product`.`tbl_sizes_idtbl_sizes` LEFT JOIN `tbl_size_categories` ON `tbl_size_categories`.`idtbl_size_categories` = `tbl_product`.`tbl_size_categories_idtbl_size_categories` WHERE `tbl_catalog_idtbl_catalog`='$productID' AND `tbl_catalog_details`.`status`=1 GROUP BY `tbl_product`.`idtbl_product`";
