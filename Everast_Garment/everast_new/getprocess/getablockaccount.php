...
$sql="";
$sql.="SELECT `tbl_subaccount`.`subaccount`, `tbl_subaccount`.`subaccountname`, `tbl_account_allocation`.`idtbl_account_allocation` FROM `tbl_subaccount` LEFT JOIN `tbl_account_allocation` ON `tbl_account_allocation`.`subaccountno`=`tbl_subaccount`.`subaccount` WHERE `tbl_account_allocation`.`tbl_company_idtbl_company`='$company' AND `tbl_account_allocation`.`tbl_company_branch_idtbl_company_branch`='$companybranch' AND `tbl_account_allocation`.`status`=2";
...
<tr>
    ...
    <td class="text-center">
        <div class="custom-control custom-checkbox">
            <input type="checkbox" class="custom-control-input checkblock" id="customCheck<?php echo $row['idtbl_account_allocation'] ?>">
