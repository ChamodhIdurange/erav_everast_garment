...
$sqlorder="SELECT `nettotal`, `remark` FROM `tbl_porder` WHERE `idtbl_porder`='$record'";
...
$roworder=$resultorder->fetch_assoc();
   ...
   $sql1="SELECT `pp`.`product_name`, `gd`.`qty`, `gd`.`idtbl_grndetail`, `g`.`idtbl_grn` FROM `tbl_grn` as `g` JOIN `tbl_grndetail` as `gd` on (`gd`.`tbl_grn_idtbl_grn` = `g`.`idtbl_grn`) JOIN `tbl_product` as `pp` ON (`pp`.`idtbl_product` = `gd`.`tbl_product_idtbl_product`) WHERE `g`.`tbl_porder_idtbl_porder` = '$record' GROUP BY `g`.`idtbl_grn`";
