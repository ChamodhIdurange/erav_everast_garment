...
$sqlorder="SELECT `p`.`total`, `p`.`discount`, `p`.`podiscount`, `p`.`nettotal`, `p`.`remark`, `c`.`name`, `c`.`phone` FROM `tbl_customer_order` as `p`  JOIN `tbl_customer` AS `c` ON (`c`.`idtbl_customer` = `p`.`tbl_customer_idtbl_customer`) WHERE `p`.`idtbl_customer_order`='$orderID'";
