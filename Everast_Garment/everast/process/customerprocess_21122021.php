...
if(!empty($_FILES["buisnesscopy"]["name"])){
    ...
    if(in_array($ext,$extension)){
        if(!file_exists("../../images/uploads/productlist/".$file_name)){
            ...
            move_uploaded_file($file_tmp=$_FILES["buisnesscopy"]["tmp_name"],"../../images/uploads/productlist/".$newFileName);
...
if(!empty($_FILES["dealerboard"]["name"])){
    ...
    if(in_array($ext,$extension)){
        if(!file_exists("../../images/uploads/productlist/".$file_name)){
            $filename=basename($file_name,$ext);
            $newFileName=md5($filename).date('Y-m-d').date('h-i-sa').$imageRandNum.".".$ext;
            move_uploaded_file($file_tmp=$_FILES["dealerboard"]["tmp_name"],"../../images/uploads/productlist/".$newFileName);
            $image_path=$target_path.$newFileName;
        }
        else{
            $filename=basename($file_name,$ext);
...
if(!empty($_FILES["selfie"]["name"])){
    ...
    if(in_array($ext,$extension)){
        if(!file_exists("../../images/uploads/productlist/".$file_name)){
            $filename=basename($file_name,$ext);
            $newFileName=md5($filename).date('Y-m-d').date('h-i-sa').$imageRandNum.".".$ext;
            move_uploaded_file($file_tmp=$_FILES["selfie"]["tmp_name"],"../../images/uploads/productlist/".$newFileName);
            $image_path=$target_path.$newFileName;
        }
        else{
            $filename=basename($file_name,$ext);
...
if(!empty($_FILES["productimage"]["name"])){
    ...
    if(in_array($ext,$extension)){
        if(!file_exists("../../images/uploads/productlist/".$file_name)){
            $filename=basename($file_name,$ext);
            $newFileName=md5($filename).date('Y-m-d').date('h-i-sa').$imageRandNum.".".$ext;
            move_uploaded_file($file_tmp=$_FILES["productimage"]["tmp_name"],"../../images/uploads/productlist/".$newFileName);
            $image_path=$target_path.$newFileName;
        }
        else{
            $filename=basename($file_name,$ext);
...
if($recordOption==1){
    ...
    if($conn->query($query)==true){
        ...
        if($cusType==2){
            $insertproductlist="INSERT INTO `tbl_customer_product`(`saleprice`, `status`, `updatedatetime`, `tbl_user_idtbl_user`, `tbl_product_idtbl_product`, `tbl_customer_idtbl_customer`) SELECT `saleprice`, 1, '$updatedatetime', $userID, `idtbl_product`, $customerID FROM `tbl_product` WHERE `status`=1";
