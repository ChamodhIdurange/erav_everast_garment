...
$joinQuery = "FROM (select `drv_doc`.`id`, `tbl_gl_receipt_details`.`cheque_no`, `tbl_gl_receipt_details`.`cheque_bank`, '' AS `branch_info`, `tbl_gl_receipt_details`.`receipt_sub_narration`, `tbl_gl_receipts`.`receipt_customer`, `tbl_gl_receipt_details`.`cheque_date`, `tbl_gl_receipt_details`.`received_amount` AS `deposit_amount`, `settle_by_cash` AS `cash_collection`, `settle_by_cheque` AS `cheque_collection` from `tbl_gl_receipt_details` INNER JOIN `tbl_gl_receipts` ON `tbl_gl_receipt_details`.`tbl_gl_receipt_id`=`tbl_gl_receipts`.`id` INNER JOIN (SELECT `tbl_gl_bank_deposit_id` AS `id`, `tbl_gl_receipt_detail_id` FROM `tbl_gl_bank_deposit_receipt_details` WHERE `deposit_cancel`=0) AS drv_doc ON `tbl_gl_receipt_details`.`id`=`drv_doc`.`tbl_gl_receipt_detail_id` WHERE `tbl_gl_receipt_details`.`settle_by_cheque`=1 AND `tbl_gl_receipt_details`.`bank_deposit`=1 UNION ALL select `id` AS `id`, '-' AS `cheque_no`, '-' AS `cheque_bank`, '-' AS `branch_info`, 'comment' AS `receipt_sub_narration`, '-' AS `receipt_customer`, '-' AS `cheque_date`, `deposit_amount`, 1 AS cash_collection, 0 AS cheque_collection FROM `tbl_gl_bank_deposits` WHERE `cash_collection`=1) AS `u`";
