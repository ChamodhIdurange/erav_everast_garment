...
$joinQuery = "FROM `tbl_invoice` AS `u` LEFT JOIN `tbl_customer` AS `ua` ON (`ua`.`idtbl_customer` = `u`.`tbl_customer_idtbl_customer`) LEFT JOIN `tbl_employee` AS `ub` ON (`ub`.`idtbl_employee` = `u`.`ref_id`) LEFT JOIN `tbl_area` AS `uc` ON (`uc`.`idtbl_area` = `u`.`tbl_area_idtbl_area`) JOIN `tbl_employee` AS `e` ON (`e`.`idtbl_employee` = `u`.`ref_id`)";

$extraWhere = "`u`.`status` IN (1, 2) and `u`.`qtycancelstatus` = '2'";
