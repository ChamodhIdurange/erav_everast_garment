...
$sqlorder="SELECT `p`.`subtotal`, `p`.`disamount`, `p`.`discount`, `p`.`nettotal`, `p`.`remark`, `c`.`name`, `c`.`phone` FROM `tbl_porder` as `p` JOIN `tbl_porder_otherinfo` AS `o` ON (`o`.`porderid` = `p`.`idtbl_porder`) JOIN `tbl_customer` AS `c` ON (`c`.`idtbl_customer` = `o`.`customerid`) WHERE `p`.`idtbl_porder`='$orderID'";
